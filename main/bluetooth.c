...
static char *bda2str(uint8_t * bda, char *str, size_t size)
{
    ...
    uint8_t *p = bda; // why? just use bda
    sprintf(str, "%02x:%02x:%02x:%02x:%02x:%02x",
            p[0], p[1], p[2], p[3], p[4], p[5]);
    return str;
}

/* bluetooth event state machine */
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_CONG_EVT:
        ...
        break;
    case ESP_SPP_WRITE_EVT: /* event automatically changes after esp_spp_write is called */
    ...
    case ESP_SPP_SRV_OPEN_EVT:
        ...
        bt_handle = param->srv_open.handle; // store handle in global variable
        vTaskResume(send_task);
        break;
